...
class WriteXmlMixin:
    ...
    def to_xml(self, encoding = "iso-8859-1"):
        ...
        #Hack to turn CDATA elements into proper <![CDATA]]> sections
        return re.sub('</CDATA>', ']]>', re.sub('<CDATA>', '<![CDATA[', f.getvalue()))

def _element(handler, name, obj, d = {}, cdata=False):
    if isinstance(obj, basestring) or obj is None:
        # special-case handling to make the API easier
        # to use for the common case.
        handler.startElement(name, d)
        if cdata:
            handler.startElement('CDATA', d)

        if obj is not None:
            handler.characters(obj)
        
        if cdata:            
            handler.endElement('CDATA')    
...
def _opt_element(handler, name, obj, cdata=False):
    if obj is None:
        return
    _element(handler, name, obj, cdata=cdata)
...
class RSS2(WriteXmlMixin):
    ...
    def publish(self, handler):
        ...
        _element(handler, "atom:link", None, {
            "href": self.feedLink,
            "rel": "self",
...
class RSSItem(WriteXmlMixin):
    ...
    def publish(self, handler):
        ...
        self.publish_extensions(handler)
        _opt_element(handler, "description", self.description, cdata=False)
        _opt_element(handler, "content:encoded", self.content, cdata=False)
